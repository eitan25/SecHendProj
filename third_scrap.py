...
def scrap_area(company, models, data):
    ...
    for model in models:
        ...
        for year in years:
            ...
            data.to_csv('./data/cars_data1605.csv', encoding="utf-8")
...
# # first iteration
# data = pd.read_csv('./data/clean_data_0705.csv', index_col=0)
# data[new_price_col] = None
# # fill in the all ready sold units with -1
# for index, row in data.iterrows():
#     if row.price0705 == -1:
#         data.at[index, new_price_col] = -1

# after block iteration
data = pd.read_csv('./data/cars_data1605.csv', index_col=0)




# check only for data not scrapped
scrapping_data = data[(data[new_price_col].isnull()) & (data.price0705 != -1)]
